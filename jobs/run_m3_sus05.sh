...
python code/densenet_train_m3_suspicious_noinv_blur_weighted_ce.py \
  ...
  --output-dir outputs/m3_suspicious05_noinv_blur \
  --epochs 30 \
  --batch-size 8 \
  --lr 1e-4 \
  --num-workers 4 \
  --seed 42 \
  --suspicious-suppression-prob 0.5 \
  --blur-kernel 21

echo "=== Training outputs ==="
ls -la outputs/m3_suspicious05_noinv_blur || true

echo "=== Config ==="
cat outputs/m3_suspicious05_noinv_blur/config.json || true

echo "=== Training history tail ==="
tail -5 outputs/m3_suspicious05_noinv_blur/training_history.csv || true

echo "=== Job finished: M3 suspicious-case05 targeted no-inversion blur ==="
date