...
echo "=== Job started: DenseNet Dynamic Smoothing + Blur + Augmentations ==="
date
echo "PWD: $(pwd)"
echo "Host: $(hostname)"
...
echo "=== Start Training ==="
python code/densenet_train_dynamic_smoothing_blur_aug.py \
  ...
  --output-dir outputs/densenet_dynamic_smoothing_blur_aug \
  --max-alpha 0.5 \
  --noise-std 0.05 \
