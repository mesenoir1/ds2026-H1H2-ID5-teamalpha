...
class KneeOAMasterDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # === DATA AUGMENTATION & MASKS (Nur im Training) ===
        if self.split == "train":
            # 1. Flip
            if random.random() < self.flip_prob:
                image_np = np.fliplr(image_np).copy()
            
            # 2. Blur (mit Maskengenerierung)
...
class DynamicHybridLoss(nn.Module):
    ...
    def forward(self, logits, features, targets, alphas, l_borders, l_rois, border_masks, out_roi_masks):
        ...
        # Überspringe Attention Loss komplett, falls Lambdas in diesem Batch alle 0 sind
...
def main():
    ...
    parser.add_argument("--output-dir", required=True, help="Der Ausgabeordner muss nun explizit angegeben werden")
    ...
    parser.add_argument("--flip-prob", type=float, default=0.5, help="Wahrscheinlichkeit für horizontalen Flip")
    parser.add_argument("--invert-prob", type=float, default=0.5, help="Wahrscheinlichkeit für Farbinvertierung")
    parser.add_argument("--noise-std", type=float, default=0.05, help="Stärke des Gaussian Noise (0.0 für aus)")
    
    # Blur
    parser.add_argument("--suppression-prob", type=float, default=0.5, help="Wahrscheinlichkeit für ROI-Blur")
