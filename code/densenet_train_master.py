...
class KneeOAMasterDataset(Dataset):
    def __init__(self, csv_path: str | Path, split: str, max_alpha: float = 0.5, 
        ...
        # Decide which type of data split csv is used (pretrained suspicious (lable smoothing) or normal)
        if "image_path" not in self.data.columns:
            if "file_name" in self.data.columns:
                ...
                raise ValueError(f"{self.csv_path} enthält weder 'image_path' noch 'file_name'.")
        ...
        if "label" in self.data.columns:
            ...
            raise ValueError(f"{self.csv_path} enthält keine gültige Label-Spalte.")
        ...
        for _, row in self.data.iterrows():
            ...
            if has_dynamic_scores:
                ...
                if not np.isnan(roi) and not np.isnan(border):
                    ...
                    if roi < 0.40: l_roi = lambda_roi_base
                    if border > 0.20: l_border = lambda_border_base
    ...
    def __getitem__(self, idx):
        ...
        label = int(row[self.label_col]) # Dynamische Spaltennutzung
...
def main():
    ...
    parser.add_argument("--train-csv", default="data/split/train.csv")
    parser.add_argument("--val-csv", default="data/split/val.csv")
    ...
    parser.add_argument("--invert-prob", type=float, default=0.5, help="Probability of Colorinversion")
    parser.add_argument("--noise-std", type=float, default=0.05, help="Gaussian Noise standard deviation (0.0 = off)")
    
    # Blur
    parser.add_argument("--suppression-prob", type=float, default=0.5, help="Probability of out of ROI-Blur")
