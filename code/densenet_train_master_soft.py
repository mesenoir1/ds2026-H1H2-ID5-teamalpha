        ...
        for _, row in self.data.iterrows():
            ...
            if has_dynamic_scores:
                ...
                if not np.isnan(roi) and not np.isnan(border):
                    ...
                    # === Cont SOFT-WEIGHTING ===
                    l_border = lambda_border_base * border
                    l_roi = lambda_roi_base * (1.0 - roi)
                    
                    # Avg Suspiciousness for Label Smoothing
                    suspiciousness = (border + (1.0 - roi)) / 2.0
                    alpha = suspiciousness * self.max_alpha
                    # ============================================
...
def main():
    ...
    parser.add_argument("--invert-prob", type=float, default=0.5, help="Invert")
