...
def apply_blur_darken_background_suppression(
    image: np.ndarray,
    roi_mask: np.ndarray,
    blur_kernel: int = 21,
    ...
    """
    Keep ROI unchanged.
    Outside ROI = blurred image darkened by darken_factor.
    """
    ...
    blurred_darkened = blurred.astype(np.float32) * darken_factor

    suppressed = (
        image.astype(np.float32) * roi_mask
        + blurred_darkened * (1.0 - roi_mask)
    )

    return np.clip(suppressed, 0, 255).astype(np.uint8)


class KneeOAM2BlurDarkenNoInvDataset(Dataset):
    def __init__(
        self,
        csv_path: str | Path,
        split: str,
    ...
    def __getitem__(self, idx):
        ...
        if self.split == "train" and self.rng.random() < self.suppression_prob:
            ...
            image_np = apply_blur_darken_background_suppression(
                image=image_np,
                roi_mask=roi_mask,
...
def main():
    ...
    parser.add_argument("--output-dir", default="outputs/m2_noinv_blur_darken")
    ...
    parser.add_argument("--darken-factor", type=float, default=0.5)
    ...
    train_dataset = KneeOAM2BlurDarkenNoInvDataset(
    ...
    val_dataset = KneeOAM2BlurDarkenNoInvDataset(
